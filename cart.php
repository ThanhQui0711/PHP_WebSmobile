...
<body>
                        ...
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <div class="product_count">
                                    <input onchange="handleChange(<?= $row['id'] ?>, <?= $item['gia'] ?>)" class="input-number" type="number" name="<?= $row['id'] ?>" value="<?= $item['soluong'] ?>" min="1" max="100"/>
                                </div>
                                </td>
                                <td>
                                <h5 class='subtotal-<?= $row['id'] ?>'><?= number_format($tong) ?>đ</h5>
                                </td>
                                <td>
                            ...
                            <tr class="bottom_button">
                                ...
                                <td>
                                    <h5 class="total"><?= number_format($tongcong) ?>đ</h5>
                                </td>
                                <td>
                                </td>
                            </tr>
  ...
  var handleChange = (key, gia) => {
    console.log(key, gia);
    var inputSoluong = document.querySelector(`.input-number[name="${key}"]`);
    var soluong = parseInt(inputSoluong.value);
    var tongtien = soluong * gia;
    var spanTongtien = document.querySelector(`.subtotal-${key}`);
    console.log("Tong tien" + spanTongtien.textContent)
    spanTongtien.textContent = tongtien.toLocaleString('vi-VN', {style: 'currency', currency: 'VND'});
...
foreach (selectAll("SELECT * FROM `taikhoan` WHERE taikhoan='$taikhoan'") as $item) {
    ...
    if (rowCount("SELECT * FROM donhang WHERE status=0 && id_taikhoan=$id_nguoidung")>0) {
        foreach (selectAll("SELECT * FROM donhang WHERE status=0 && id_taikhoan=$id_nguoidung") as $item) {
            ...
            if (rowCount("SELECT * FROM ctdonhang WHERE id_donhang = $idDhcu && id_sanpham = 3")>0) {
                selectAll("UPDATE ctdonhang SET soluong= 50 WHERE id_donhang = $idDhcu && id_sanpham = 3"); 
            }
        }
        }
